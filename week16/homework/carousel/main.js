...
class Carousel {
  ...
  render() {
    ...
    let stopBtn = <button disabled>stop</button>;
    let resumeBtn = <button disabled>resume</button>;
    let restartBtn = <button disabled>restart</button>;
    let isStop = false;
    let root = (
      <div class="container">
    ...
    let nextPic = () => {
      ...
      setTimeout(function () {
        ...
        tl.add(
          new Animation(next.style, 'transform', 0, 100, 1000, 0, ease, (v) => {
            if (v === 100) {
              secondStatus = true;
              if (firstStatus) {
                position = nextPosition;
    ...
    root.addEventListener('panstart', (event) => {
      ...
      let up = (event) => {
        ...
        root.removeEventListener('pan', move);
        root.removeEventListener('panend', up);
      };
      root.addEventListener('pan', move);
      root.addEventListener('panend', up);
    });
